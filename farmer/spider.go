...
func (spider *Spider) launch() {
	// bin, _ := os.LookupEnv("BROWSER_BIN")
	// u := launcher.New().MustLaunch()
	// u := launcher.New().Bin("/browser/").Set("disable-gpu").Set("no-sandbox").MustLaunch()
	spider.engine.MustConnect()
	// spider.engine.ControlURL(u).MustConnect().MustIncognito().NoDefaultDevice()
...
func (spider *Spider) Info(v ...any) {
	spider.logger.Info(v...)
}

func (spider *Spider) Warn(v ...any) {
	spider.logger.Warn(v...)
}
...
func (spider *Spider) retrieveAll(page *rod.Page) {
	...
	for _, job := range spider.jobs {
		if page.MustHas(job.el) {
			el := page.Timeout(15 * time.Second).MustElement(job.el)
			if el != nil {
				res := job.callback(el)
...
func (spider *Spider) getPage() *rod.Page {
	...
	page, err := spider.pool.Get(instance)
	if err != nil {
		panic(err)
	}
	return page
...
func (spider *Spider) visit(urls []string, depth int) {
	...
	run := func(url string) {
		...
		spider.Info("got the page, waiting for it to be loaded ", page)
